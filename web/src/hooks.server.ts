...
import type { AuthErrorDetails, AuthResponse } from '$lib/schemas/auth-response'
...
export const handle: Handle = async ({ event, resolve }) => {
  try {
    ...
    const result = (await response.json()) as Response<AuthResponse, AuthErrorDetails>
    ...
    if (response.status === 401) {
      ...
      const retryResponse = (await response.json()) as Response<AuthResponse, AuthErrorDetails>
